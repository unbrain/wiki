...
// ---------- 主流程 ----------
const api = await fetchCodetop()
const byId = new Map(api.map((q) => [String(q.leetcode.frontend_question_id), q]))
...
if (lastToday) {
  ...
  const window = api.slice(NEW_FROM - 1, NEW_FROM - 1 + NEW_RANGE)
  newQuestions = []
...
// --- 生成笔记 ---
const fmtNew = (q) => {
  const { frontend_question_id: id, title, level } = q.leetcode
  const slug = q.leetcode.slug_title
  const mark = done.has(String(id)) ? '🔁（已刷过，重做）' : '🆕'
  return `| [${id}. ${title}](https://leetcode.cn/problems/${slug}/) | ${levelName(level)} | ${q.value} | ${mark} |`
}
const fmtReview = (r) => {
...
const pickId = (q) => String(q.leetcode.frontend_question_id)
console.log(`✅ 新题: ${newQuestions.map((q) => `${pickId(q)}.${q.leetcode.title}${done.has(pickId(q)) ? '(重做)' : ''}`).join(' / ')}`)
